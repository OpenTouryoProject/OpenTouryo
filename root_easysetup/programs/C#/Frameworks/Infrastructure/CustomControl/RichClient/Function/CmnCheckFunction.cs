...
namespace Touryo.Infrastructure.CustomControl.RichClient
{
    ...
    public class CmnCheckFunction
    {
        ...
        /// のエラー メッセージ
        /// </summary>
        public const string IsIndispensabileCheckErrorMessage = "必須チェック エラー";
        ...
        public static bool HasErrors(Control parentCtrl, List<CheckResult> lstCheckResult)
        {
            ...
            // チェック対象のコントロールなら、
            if (parentCtrl is WinCustomTextBox
                || parentCtrl is WinCustomMaskedTextBox)
            {
                // チェックし、
                string[] temp = null;
                ...
                {
                    // エラーならエラー情報を保持する。
                    CheckResult cr = new CheckResult(parentCtrl.Name);
