...
**Optional表記:**  
以下、optional表記のある対象RDBMSのデータプロバイダとDBライブラリは、<a href="#プログラムのビルド">デフォルトのビルドスクリプトのビルドプロセス</a>、または[Open 棟梁のNuGetパッケージ](https://www.nuget.org/packages?q=OpenTouryo)に含まれません。必要に応じてデータプロバイダを自分でダウンロードし、Open 棟梁のデータアクセス用のプロジェクト (DamXXX.csproj) から、各データプロバイダに参照設定を張り直し、これらのライブラリをビルドして利用してください。
