...
Namespace Touryo.Infrastructure.Business.Workflow
	...
	Public Class Workflow
        ...
        Public Function StartWorkflow(_startWorkflow As DataRow, workflowControlNo As String, fromUserId As Decimal, toUserId As Decimal, workflowReserveArea As String, currentWorkflowReserveArea As String, _
         replyDeadline As System.Nullable(Of DateTime)) As Integer
            '#Region "チェック処理を実装"
            ...
            Dim toUserInfo As String = String.Empty
            If toUserId = 0 Then
                toUserInfo = Workflow.GetUserInfo(CDec(_startWorkflow("ToUserId")))
                toUserId = CDec(_startWorkflow("ToUserId"))
            Else
		...
		''' </summary>
		Public Sub TurnbackSlipIssuanceUserID(subsystemId As String, workflowControlNo As String, fromUserId As Decimal, toUserId As System.Nullable(Of Decimal), currentWorkflowReserveArea As String)
			'#Region "チェック処理を実装"

			Dim toUserInfo As String = ""

			'If ToUserId is not null ans ActionType is End then getting user information
			If toUserId.HasValue Then
				' Endの時は不要 
				toUserInfo = Workflow.GetUserInfo(toUserId.Value)
			End If
			...
			Dim dt As DataTable = GetSlipIssuanceUserID(subsystemId, workflowControlNo)
			...
			daoT_CurrentWorkflow.Set_FromUserId_forUPD = toUserId
			' 実際のユーザIDを入力する。
		...
		''' </summary>
		Private Function GetSlipIssuanceUserID(subSystemId As String, workflowControlNo As String) As DataTable
			'#Region "チェック処理を実装"
		...
		''' <returns></returns>
		Public Function ForcedTermination(nextWorkflow As DataRow, workflowControlNo As String, fromUserId As Decimal, currentWorkflowReserveArea As String) As Integer
			'#Region "チェック処理を実装"
		...
		''' </summary>
		Public Sub SwitchPersonInCharge(nextWorkflow As DataRow, workflowControlNo As String)
