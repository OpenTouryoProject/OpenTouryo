...
//**********************************************************************************
//* クラス名        ：DamHiRDB
//* クラス日本語名  ：データアクセス・プロバイダ＝HiRDBのデータアクセス制御クラス
//*
//* 作成者          ：生技 西野
//* 更新履歴        ：
//* 
...
//*  2015/07/05  Sai              Implemented virtual property of IDbCommand in DamHiRDB class
...
{
    /// <summary>データアクセス・プロバイダ＝HiRDBのデータアクセス制御クラス</summary>
    /// <remarks>必要なメソッド・プロパティを利用する</remarks>
    public class DamHiRDB : BaseDam
    {
        #region クラス変数
        ...
        private HiRDBConnection _cnn;

        /// <summary>トランザクション</summary>
        private HiRDBTransaction _tx;

        /// <summary>コマンド</summary>
        private HiRDBCommand _cmd;

        /// <summary>アダプタ</summary>
        private HiRDBDataAdapter _adpt;
        ...
        /// <summary>HiRDBConnection（読み取り専用）</summary>
        /// <remarks>必要に応じて利用する。</remarks>
        public HiRDBConnection DamHiRDBConnection
        {
            get
            {
                // コネクションを戻す
        ...
        /// <summary>HiRDBDataAdapter（読み取り専用）</summary>
        /// <remarks>
        /// 利用時、コネクション、トランザクションが有効な状態になっている事
        /// 必要に応じて利用する。
        /// </remarks>
        public HiRDBDataAdapter DamHiRDBDataAdapter
        {
            get
            {
                // コネクション、トランザクションを設定
                ...
                this._adpt = new HiRDBDataAdapter(this._cmd);
        ...
        /// <summary>HiRDBCommand（読み取り専用）</summary>
        /// <remarks>必要に応じて利用する。</remarks>
        public HiRDBCommand DamHiRDBCommand
        {
            get
            {
                // コマンドを戻す
        ...
        /// <summary>HiRDBTransaction（読み取り専用）</summary>
        /// <remarks>必要に応じて利用する。</remarks>
        public HiRDBTransaction DamHiRDBTransaction
        {
            get
            {
                // トランザクションを戻す
        ...
        public override void BeginTransaction(DbEnum.IsolationLevelEnum iso)
        {
            ...
            else if (iso == DbEnum.IsolationLevelEnum.Snapshot)
            {
                ...
                throw new ArgumentException(String.Format(
                    PublicExceptionMessage.DB_ISO_LEVEL_PARAM_ERROR_SS, "HiRDB"));
            }
            ...
            object dbTypeInfo, int size, ParameterDirection paramDirection)
        {
            ...
            else
            {
                ...
                HiRDBParameter param = new HiRDBParameter();
                ...
                if (dbTypeInfo != null)
                {
                    ...
                    {
                        // OFF扱い = HiRDBType
                        param.HiRDBType = (HiRDBType)dbTypeInfo;
                    }
                }
        ...
        private void PreExecQuery()
        {
            if (this.IsDPQ)
            {
                ...
                this._cmd.CommandText = this.Convert(DamHiRDB._paramSign);
        ...
        private void ChangesToOrderBind()
        {
            ...
            foreach (HiRDBParameter parameter in this._cmd.Parameters)
            {
            ...
            while(true)
            {
                ...
                paramName = this.GetParamByText(this._cmd.CommandText, DamHiRDB._paramSign, out paramSignIndex);
                ...
                //this._cmd.CommandText = this._cmd.CommandText.Replace(DamHiRDB._paramSign + paramName, "?");
                ...
                else
                {
                    ...
                    throw new ArgumentException(string.Format(PublicExceptionMessage.ORDER_BIND_ERROR_PARAMETER_NOT_FOUND, DamHiRDB._paramSign + paramName));
                }
            }
        }
        ...
        /// <summary>Insert、Update、Deleteクエリを実行し、影響を受けた行数を返す。</summary>
        /// <param name="nArraySize">配列バインド時の配列数</param>
        /// <returns>影響を受けた行数</returns>
        /// <remarks>
        /// HiRDBCommandのExecuteNonQueryを実行する。
        /// 通常、Ｄａｏ経由で利用する。
        ...
        public int ExecInsUpDel_NonQuery(int nArraySize)
        {
            ...
            // SQLを実行して、影響を受けた行数を返す。
            return this._cmd.ExecuteNonQuery(nArraySize);
        }
        ...
        public override string GetCurrentQueryForLog()
        {
            ...
            else if (GetConfigParameter.GetConfigValue(PubLiteral.SQL_TRACELOG).ToUpper() == PubLiteral.ON)
            {
                ...
                // パラメタを取得
                foreach (HiRDBParameter p in this._cmd.Parameters)
                {
