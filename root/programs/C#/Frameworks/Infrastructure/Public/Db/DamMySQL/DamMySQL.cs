...
//**********************************************************************************
//* クラス名        ：DamMySQL
//* クラス日本語名  ：データアクセス・プロバイダ＝MySQL Connector/NETのデータアクセス制御クラス
//*
//* 作成者          ：生技 西野
//* 更新履歴        ：
//* 
...
//*  2015/07/05  Sai              Implemented virtual property of IDbCommand in DamMySQL class
//**********************************************************************************

// データアクセスプロバイダ（Connector/NET）
...
namespace Touryo.Infrastructure.Public.Db
{
    ...
    public class DamMySQL : BaseDam
    {
        ...
        private MySqlDataAdapter _adpt;
        ...
        public MySqlConnection DamMySqlConnection
        {
            get
            {
                // コネクションを戻す
        ...
        /// 必要に応じて利用する。
        /// </remarks>
        public MySqlDataAdapter DamMySqlDataAdapter
        {
            get
            {
                // コネクション、トランザクションを設定
                ...
                this._adpt = new MySqlDataAdapter(this._cmd);
        ...
        /// <remarks>必要に応じて利用する。</remarks>
        public MySqlCommand DamMySqlCommand
        {
            get
            {
                // コマンドを戻す
        ...
        /// <remarks>必要に応じて利用する。</remarks>
        public MySqlTransaction DamMySqlTransaction
        {
            get
            {
                // トランザクションを戻す
        ...
        {
            // コネクションをオープン
            this._cnn = new MySqlConnection(connstring);
            this._cnn.Open();
        }
        ...
        public override void BeginTransaction(DbEnum.IsolationLevelEnum iso)
        {
            ...
            else if (iso == DbEnum.IsolationLevelEnum.Snapshot)
            {
                ...
                throw new ArgumentException(String.Format(
                    PublicExceptionMessage.DB_ISO_LEVEL_PARAM_ERROR_SS, "MySQL"));
            }
            ...
            object dbTypeInfo, int size, ParameterDirection paramDirection)
        {
            ...
            else
            {
                ...
                if (dbTypeInfo != null)
                {
                    ...
                    {
                        // OFF扱い = MySqlDbType
                        param.MySqlDbType = (MySqlDbType)dbTypeInfo;
                    }
                }
        ...
        private void PreExecQuery()
        {
            if (this.IsDPQ)
            {
                ...
                this._cmd.CommandText = this.Convert(DamMySQL._paramSign);
        ...
        public override string GetCurrentQueryForLog()
        {
            ...
            else if (GetConfigParameter.GetConfigValue(PubLiteral.SQL_TRACELOG).ToUpper() == PubLiteral.ON)
            {
                ...
                // パラメタを取得
                foreach (MySqlParameter p in this._cmd.Parameters)
                {
