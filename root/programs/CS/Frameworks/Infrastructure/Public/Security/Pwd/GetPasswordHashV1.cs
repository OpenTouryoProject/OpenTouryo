...
//* クラス日本語名  ：Password Hashを取得するクラス（v1
//*
//* 作成者          ：生技 西野
//* 更新履歴        ：
//*
...
//*  2017/01/10  西野 大介         Stretch回数とsaltのpublic property procedureを追加
...
{
    /// <summary>Password Hashを取得するクラス（v1</summary>
    ...
    public class GetPasswordHashV1
    {
        ...
        /// <param name="saltedPassword">Salted and hashed password.</param>
        /// <param name="ekha">Hashアルゴリズム列挙型</param>
        /// <param name="han">Hashアルゴリズム名（Stretching用）</param>
        /// <returns>
        /// true：パスワードは一致した。
        /// false：パスワードは一致しない。
        /// </returns>
        public static bool EqualSaltedPassword(string rawPassword, string saltedPassword, EnumKeyedHashAlgorithm ekha, HashAlgorithmName han)
        {
            // salt部分を取得
            string[] temp = saltedPassword.Split('.');
            ...
            // 引数のsaltedPasswordと、rawPasswordから自作したsaltedPasswordを比較
            if (hashedPassword == GetPasswordHashV1.GetKeyedHashString(salt + rawPassword, ekha, key, saltByte, stretchCount, han))
            {
                // 一致した。
                return true;
            }
        ...
        /// <summary>文字列のHash値を計算して返す。</summary>
        /// <param name="ss">文字列</param>
        /// <param name="ekha">Keyed Hashアルゴリズム列挙型</param>
        /// <param name="password">使用するパスワード</param>
        /// <returns>Hash値（文字列）</returns>
        ...
        {
            return GetPasswordHashV1.GetKeyedHashString(ss, ekha, password, salt, GetPasswordHashV1.StretchCount);
        }
        ...
        /// <param name="password">使用するパスワード</param>
        /// <param name="salt">salt</param>
        /// <param name="stretching">Stretch回数</param>
        /// <returns>Hash値（文字列）</returns>
        private static string GetKeyedHashString(string ss, EnumKeyedHashAlgorithm ekha, string password, byte[] salt, int stretching)
        {
            return GetPasswordHashV1.GetKeyedHashString(ss, ekha, password, salt, GetPasswordHashV1.StretchCount, HashAlgorithmName.SHA256);
        }
        ...
        /// <param name="han">Hashアルゴリズム名（Stretching用）</param>
        /// <returns>Hash値（文字列）</returns>
        private static string GetKeyedHashString(
            string ss, EnumKeyedHashAlgorithm ekha, string password, byte[] salt, int stretching, HashAlgorithmName han)
        {
            return CustomEncode.ToBase64String(
                GetKeyedHashBytes(CustomEncode.StringToByte(ss, CustomEncode.UTF_8), ekha, password, salt, stretching, han));
        }
        ...
        /// <summary>Byte配列のHash値を計算して返す。</summary>
        /// <param name="asb">Byte配列</param>
        /// <param name="ekha">Keyed Hashアルゴリズム列挙型</param>
        /// <param name="password">使用するパスワード</param>
        /// <returns>Hash値（Byte配列）</returns>
        ...
        {
            return GetPasswordHashV1.GetKeyedHashBytes(asb, ekha, password, salt, GetPasswordHashV1.StretchCount);
        }
        ...
        {
            return GetPasswordHashV1.GetKeyedHashBytes(asb, ekha, password, salt, stretchCount, HashAlgorithmName.SHA256);  
        }
        ...
        /// <param name="han">Hashアルゴリズム名（Stretching用）</param>
        /// <returns>Hash値（Byte配列）</returns>
        private static byte[] GetKeyedHashBytes(
            byte[] asb, EnumKeyedHashAlgorithm ekha, string password, byte[] salt, int stretchCount, HashAlgorithmName han)
        {
            // キーを生成する。
            Rfc2898DeriveBytes passwordKey = new Rfc2898DeriveBytes(password, salt, stretchCount, han);
            ...
            // MACTripleDES ：長さが 16 または 24 Byteのキーを受け入れる

            // Keyed Hashサービスプロバイダを生成
